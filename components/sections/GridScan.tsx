...
function smoothDampVec2(
  ...
  const adjustedTarget = current.clone().sub(change);
  ...
  out.copy(adjustedTarget.clone().add(change.add(temp).multiplyScalar(exp)));
...
function smoothDampFloat(
  ...
  let out = adjustedTarget + (change + temp) * exp;
...
export default function GridScan({
  ...
  }, [enablePost, bloomThreshold, bloomSmoothing]);

  // Uniform hot-updates (no renderer rebuild needed)
  useEffect(() => {
    ...
    bloomIntensity, bloomThreshold, bloomSmoothing, chromaticAberration,
    noiseIntensity, scanGlow, scanOpacity, scanDirection, scanSoftness,
