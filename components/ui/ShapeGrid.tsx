...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            const flip = ((col + colShift + row) % 2 + 2) % 2 !== 0;
            const cellKey = `${col},${row}`;
            const alpha = cellOpacities.current.get(cellKey);
            if (alpha) {
              ctx.globalAlpha = alpha;
      ...
      gradient.addColorStop(1, 'rgba(6, 9, 18, 0.85)');
    ...
    const getCell = (mouseX: number, mouseY: number) => {
      if (isHex) {
        const colShift = Math.floor(gridOffset.current.x / hexHoriz);
        const offsetX = ((gridOffset.current.x % hexHoriz) + hexHoriz) % hexHoriz;
        const offsetY = ((gridOffset.current.y % hexVert) + hexVert) % hexVert;
      ...
      if (isTri) {
        ...
        const adjustedX = mouseX - offsetX;
        const adjustedY = mouseY - offsetY;
        return { x: Math.round(adjustedX / halfW), y: Math.floor(adjustedY / squareSize) };
      }
      const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
      const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
      const adjustedX = mouseX - offsetX;
      const adjustedY = mouseY - offsetY;
      if (shape === 'circle') {
        return { x: Math.round(adjustedX / squareSize), y: Math.round(adjustedY / squareSize) };
      }
      return { x: Math.floor(adjustedX / squareSize), y: Math.floor(adjustedY / squareSize) };
    };
